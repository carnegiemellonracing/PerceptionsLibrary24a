...
std::vector<std::pair<Point, std::string>> ClusterCones(const std::vector<Point>& points) {
    ...
    auto clusters = NewRunDBSCAN(points, 5, 3);
    ...
    std::cout << "Number of points in the track cluster: " << num_points << std::endl;
    ...
    if (min_positive_x_dist == INFINITY || min_negative_x_dist == INFINITY) {
        throw std::runtime_error("Unable to find starting point: Missing points in positive or negative x.");
    }
    ...
    std::cout << "Starting reference point: (" << current_ref.x << ", "
              << current_ref.y << ", " << current_ref.z << ")" << std::endl;

    // Step 5: Section points based on the updated logic
    std::vector<std::vector<Point>> sections;
    ...
    thrust::device_vector<Point> d_midline(num_sections);

    // Compute midline points in parallel
    computeMidlinePoints<<<(num_sections + 255) / 256, 256>>>(
    ...
    // Classify cones based on the midline
